...
void DriveDelegate::paint(QPainter *p, const QStyleOptionViewItem &opt, const QModelIndex &idx) const
{
    ...
    if (m_showBars && (path.startsWith("/") || (isKioPath && total > 0))) {
        if (total > 0) {
            paintStorageBar(p, r, idx, name, path);
        }
    } else if (isKioPath) {
        paintRemoteSubtitle(p, r, name, path);
    } else {
        paintUnmountedLocal(p, r, name, path);
    }
    p->restore();
...
void DriveDelegate::paintBackground(QPainter *p, const QStyleOptionViewItem &opt, const QModelIndex &idx) const
{
...
void DriveDelegate::paintIcon(QPainter *p, const QRect &r, const QIcon &icon) const
{
...
void DriveDelegate::paintStorageBar(QPainter *p, const QRect &r, const QModelIndex &idx, const QString &name, const QString &path) const
{
    const double total = idx.data(Qt::UserRole + 10).toDouble();
    ...
    const bool isKioPath = path.contains(QStringLiteral(":/")) && !path.startsWith(QStringLiteral("/"))
                           && !path.startsWith(QStringLiteral("solid:")) && !path.startsWith(QStringLiteral("file:"));

    QString ipStr = buildIpString(path, isKioPath);
    ...
    p->drawText(textX, textY, nameDrawW, lineH, Qt::AlignLeft | Qt::AlignVCenter, fm.elidedText(name, Qt::ElideRight, nameDrawW));

    if (!ipStr.isEmpty() && ipInlineW > 0) {
        const int ipAvailW = sizeX - ipInlineX - 4;
        if (ipAvailW > 20) {
            p->setPen(QColor(TM().colors().textMuted));
            p->drawText(ipInlineX, textY, ipAvailW, lineH, Qt::AlignLeft | Qt::AlignVCenter, fm.elidedText(ipStr, Qt::ElideRight, ipAvailW));
        }
    }
...
void DriveDelegate::paintRemoteSubtitle(QPainter *p, const QRect &r, const QString &name, const QString &path) const
{
    const int textX   = r.left() + 40;
    const int textW   = r.width() - 50;
    ...
    if (subtitle.isEmpty()) {
        ...
        p->setPen(QColor(TM().colors().textPrimary));
        p->drawText(textX, r.top(), textW, lineH, Qt::AlignLeft | Qt::AlignVCenter, fm.elidedText(name, Qt::ElideRight, textW));
        { QFont _f = qApp->font(); _f.setPointSize(8); p->setFont(_f); }
        p->setPen(QColor(TM().colors().textAccent));
        p->drawText(textX, r.top() + lineH, textW, lineH, Qt::AlignLeft | Qt::AlignVCenter, QFontMetrics(p->font()).elidedText(subtitle, Qt::ElideRight, textW));
    }
}

void DriveDelegate::paintUnmountedLocal(QPainter *p, const QRect &r, const QString &name, const QString &path) const
{
    const int textX   = r.left() + 40;
    const int textW   = r.width() - 50;
...
QString DriveDelegate::buildIpString(const QString& path, bool isKioPath) const
{
    QString ipStr;
