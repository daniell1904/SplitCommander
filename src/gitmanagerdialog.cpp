...
void GitManagerDialog::buildUI() {
  ...
  const QString inputStyle =
      QString(
          ...
          .arg(TM().colors().bgInput, TM().colors().textPrimary,
               TM().colors().borderAlt);
  const QString btnBase =
      "QPushButton { border-radius:3px; padding:2px 10px; font-size:12px; "
      "font-weight:bold; min-height:22px; }";

  const QString gitNormBtnSS =
      btnBase + QString("QPushButton { background:%1; color:%2; border:1px "
                        "solid %3; } QPushButton:hover { background:%4; }")
                    .arg(TM().colors().bgPanel, TM().colors().textPrimary,
                         TM().colors().borderAlt, TM().colors().bgHover);
  const QString gitPrimBtnSS =
      btnBase + QString("QPushButton { background:%1; color:%2; border:none; } "
                        "QPushButton:hover { background:%3; }")
                    .arg(TM().colors().accent, TM().colors().textLight,
                         TM().colors().accentHover);

  // --- COMPACT ACTION GRID ---
  auto addCompactBtn = [&](const QString &title, const QString &tooltip,
                           const QString &style,
  ...
  m_gitBranchLabel->setStyleSheet("color: " + TM().colors().accent +
  ...
  // 3. ACTIONS
  auto *actionLbl = new QLabel(
      tr("<b>3. Aktionen (Tipp: Maus über Button halten für Hilfe)</b>"));
  ...
  // Row 0
  actionGrid->addWidget(
      addCompactBtn(
          tr("Hochladen (Push)"),
          tr("Sendet deine Änderungen an GitHub, damit sie dort gespeichert "
             "sind."),
          gitPrimBtnSS,
          [this]() {
            QString msg = m_gitCommitMsg->text().trimmed();
            if (msg.isEmpty())
              msg = "Automatisches Speichern";
            runGitCommand({"add", "."});
            ...
            if (m_createReleaseOpt->isChecked()) {
              ...
              QString tag = QInputDialog::getText(
                  this, tr("GitHub Release"), tr("Tag Name für das Release:"),
                  QLineEdit::Normal, "", &ok);
              if (ok && !tag.isEmpty())
            ...
            QMessageBox::information(
                this, tr("Fertig"),
                tr("Deine Änderungen wurden erfolgreich hochgeladen!"));
          }),
      0, 0);

  actionGrid->addWidget(
      addCompactBtn(
          tr("Aktualisieren (Pull)"),
          tr("Holt die neuesten Änderungen von GitHub auf deinen Computer."),
          gitNormBtnSS, [this]() { runGitCommand({"pull"}); }),
      0, 1);

  actionGrid->addWidget(
      addCompactBtn(
          tr("Unterschiede (Diff)"),
          tr("Zeigt dir genau an, welche Zeilen im Code du geändert hast."),
          gitNormBtnSS, [this]() { runGitCommand({"diff"}); }),
      0, 2);
  ...
  actionGrid->addWidget(
      addCompactBtn(
          tr("Verlauf (Log)"),
          tr("Zeigt eine Liste der letzten Änderungen und wer sie gemacht "
             "hat."),
          gitNormBtnSS,
          [this]() { runGitCommand({"log", "--oneline", "-n", "20"}); }),
      1, 0);

  actionGrid->addWidget(addCompactBtn(tr("Tag erstellen"),
                                      tr("Markiert den aktuellen Stand als "
                                         "eine feste Version (z.B. v1.0)."),
                                      gitNormBtnSS,
                                      [this]() {
                                        bool ok;
                                        QString name = QInputDialog::getText(
                                            this, tr("Tag erstellen"),
                                            tr("Name für den Tag:"),
                                            QLineEdit::Normal, "", &ok);
                                        if (ok && !name.isEmpty())
                                          runGitCommand({"tag", name});
                                      }),
                        1, 1);

  actionGrid->addWidget(
      addCompactBtn(tr("Branch wechseln"),
                    tr("Erstellt einen neuen Zweig für Experimente oder "
                       "wechselt den aktuellen."),
                    gitNormBtnSS,
                    [this]() {
                      bool ok;
                      QString name = QInputDialog::getText(
                          this, tr("Branch wechseln/erstellen"),
                          tr("Name des Branch:"), QLineEdit::Normal, "", &ok);
                      if (ok && !name.isEmpty())
  ...
  // Row 2
  actionGrid->addWidget(
      addCompactBtn(tr("Stash (Parken)"),
                    tr("Verschiebt deine aktuellen Änderungen in eine "
                       "Warteschlange, um später daran weiterzuarbeiten."),
                    gitNormBtnSS, [this]() { runGitCommand({"stash"}); }),
      2, 0);
  ...
  m_pushTagsOpt->setStyleSheet("color: " + TM().colors().textPrimary +
                               "; font-size: 11px;");
  m_createReleaseOpt = new QCheckBox(tr("Release auf GitHub erstellen"));
  m_createReleaseOpt->setStyleSheet("color: " + TM().colors().textPrimary +
                                    "; font-size: 11px;");
  ...
  line->setStyleSheet(
      QString("background: %1; max-height: 1px; margin: 10px 0;")
  ...
  addCompactBtn(
      tr("Alles Verwerfen"),
      tr("Löscht ALLE deine ungespeicherten Änderungen und setzt alles auf den "
         "Stand von GitHub zurück. Vorsicht!"),
      btnBase + "QPushButton { color:#ff6b6b; border:1px solid #ff6b6b; }",
      [this]() {
        if (QMessageBox::critical(this, tr("Vorsicht"),
                                  tr("Willst du wirklich alle deine Änderungen "
  ...
  form->setSpacing(6);
  m_gitLocalDir = new QLineEdit();
  m_gitLocalDir->setStyleSheet(inputStyle);
  ...
  m_gitRemoteUrl = new QLineEdit();
  m_gitRemoteUrl->setStyleSheet(inputStyle);
  form->addRow(tr("GitHub Link:"), m_gitRemoteUrl);

  m_gitUsername = new QLineEdit();
  m_gitUsername->setStyleSheet(inputStyle);
  form->addRow(tr("GitHub Name:"), m_gitUsername);

  m_gitToken = new QLineEdit();
  m_gitToken->setEchoMode(QLineEdit::Password);
  m_gitToken->setStyleSheet(inputStyle);
  ...
  connect(btnBrowse, &QPushButton::clicked, this, [this]() {
    QString dir = QFileDialog::getExistingDirectory(this, tr("Ordner wählen"),
                                                    m_gitLocalDir->text());
    if (!dir.isEmpty()) {
  ...
  m_gitLog->setStyleSheet(QString("background:%1; color:%2; border:2px solid "
                                  "%3; font-family:monospace; font-size:11px; "
  ...
  const QString footerBtnBase =
      "QPushButton { border-radius: 4px; padding: 4px 16px; font-size: 13px; "
      "font-weight: bold; min-height: 22px; }";
...
void GitManagerDialog::runGitCommand(const QStringList &args) {
  ...
  // Wenn Token vorhanden und wir pushen/pullen, versuchen wir die URL mit Token
  // zu nutzen
...
void GitManagerDialog::refreshGitStatus() {
  if (m_gitPath.isEmpty()) {
    m_gitLog->append("<span style='color:#ffaa00;'>Warnung: Kein "
                     "Projekt-Ordner festgelegt.</span>");
    return;
  }
  ...
  if (!proc.waitForFinished(5000)) {
    m_gitLog->append("<span style='color:#ff6b6b;'>Fehler: 'git status' konnte "
                     "nicht ausgeführt werden (Timeout).</span>");
    return;
  }
  ...
  if (!err.isEmpty()) {
    m_gitLog->append("<span style='color:#ff6b6b;'>Git Fehler: " + err +
                     "</span>");
  }
  ...
  if (out.isEmpty()) {
    ...
    // Debug info in log
    m_gitLog->append("<i>Status-Check in: " + m_gitPath +
                     " (Alles aktuell)</i>");
    return;
  }
  ...
  for (const QString &line : lines) {
    if (line.length() >= 3) {
      ...
      auto *item = new QListWidgetItem(QString("[%1] %2").arg(status, file),
                                       m_gitStatusList);
...
void GitManagerDialog::createGitHubRelease(const QString &tag) {
  ...
  QString json = QString("{\"tag_name\":\"%1\",\"name\":\"%1\",\"body\":"
                         "\"Release %1\",\"draft\":false,\"prerelease\":false}")
                     .arg(tag);
  ...
  args << "-L" << "-X" << "POST"
       ...
       << "-H" << QString("Authorization: Bearer %1").arg(token) << "-H"
  ...
  connect(proc, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
          this, [this, proc, tag]() {
            ...
            QString err = QString::fromUtf8(proc->readAllStandardError());
            if (!err.isEmpty())
              m_gitLog->append("<span style='color:#ff6b6b;'>" + err +
                               "</span>");
