...
QToolButton *PaneToolbar::makeToolButton(const QString &icon, const QString &tip, void (PaneToolbar::*sig)())
{
...
void PaneToolbar::buildRow1(QVBoxLayout *vlay)
{
    // Zeile 1: Pfad-Label und Aktions-Buttons
    ...
    r1->addStretch(1);
    r1->addWidget(makeToolButton("view-sort-ascending", tr("Sortieren"), &PaneToolbar::sortClicked));

    m_newFolderBtn = makeToolButton("folder-new", tr("Neu"), &PaneToolbar::newFolderClicked);
    r1->addWidget(m_newFolderBtn);

    m_emptyTrashBtn = makeToolButton("trash-empty", tr("Papierkorb leeren"), &PaneToolbar::emptyTrashClicked);
    m_emptyTrashBtn->hide();
    r1->addWidget(m_emptyTrashBtn);
    vlay->addLayout(r1);
}

void PaneToolbar::buildRow2(QVBoxLayout *vlay)
{
    // Zeile 2: Anzahl, Auswahl und Größe
...
void PaneToolbar::buildRow3(QVBoxLayout *vlay)
{
    // Zeile 3: Navigation und Ansichtsmodi
    auto *r3 = new QHBoxLayout();
    r3->setContentsMargins(0, 0, 0, 0);
    r3->setSpacing(2);

    m_backBtn = makeToolButton("go-previous", tr("Zurück"), &PaneToolbar::backClicked);
    m_backBtn->setEnabled(false);
    r3->addWidget(m_backBtn);

    m_fwdBtn = makeToolButton("go-next", tr("Vorwärts"), &PaneToolbar::forwardClicked);
    m_fwdBtn->setEnabled(false);
    r3->addWidget(m_fwdBtn);
    r3->addWidget(makeToolButton("go-up", tr("Hoch"), &PaneToolbar::upClicked));
