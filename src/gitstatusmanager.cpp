...
void GitStatusManager::setupWatchers() {
    ...
    if (mode != QStringLiteral("onchange")) return;

    // Repo-Ordner rekursiv beobachten (nur Top-Level + 1-2 Ebenen wegen Limits)
    for (const auto &r : Config::gitRepos()) {
        if (r.localDir.isEmpty() || !QFileInfo::exists(r.localDir)) continue;
...
void GitStatusManager::refreshAllRepos() {
    for (const auto &r : Config::gitRepos()) {
        if (!r.localDir.isEmpty()) refreshRepo(r.localDir);
    }
}

void GitStatusManager::refreshRepo(const QString &repoPath) {
    if (repoPath.isEmpty() || !QFileInfo::exists(repoPath)) return;
    runStatus(repoPath);
...
void GitStatusManager::runStatus(const QString &repoPath) {
    ...
    connect(proc, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
            this, [this, proc, repoPath](int, QProcess::ExitStatus) {
        ...
        for (auto it = repoMap.begin(); it != repoMap.end(); ) {
            if (it.value() == GitFileStatus::LocalChange) it = repoMap.erase(it);
            else ++it;
        }
...
void GitStatusManager::runFetchThenDiff(const QString &repoPath) {
    ...
    connect(fetch, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
            this, [this, fetch, repoPath](int, QProcess::ExitStatus) {
        fetch->deleteLater();
        // Jetzt diff gegen Upstream
        auto *diff = new QProcess(this);
        diff->setWorkingDirectory(repoPath);
        connect(diff, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
                this, [this, diff, repoPath](int code, QProcess::ExitStatus) {
            const QString out = QString::fromUtf8(diff->readAllStandardOutput());
            auto &repoMap = m_status[repoPath];
            // Vorher RemoteAhead entfernen
            for (auto it = repoMap.begin(); it != repoMap.end(); ) {
                if (it.value() == GitFileStatus::RemoteAhead) it = repoMap.erase(it);
                else ++it;
            }
            if (code == 0) {
        ...
        diff->start("git", {"diff", "--name-only", "HEAD..@{u}"});
    });
...
GitFileStatus GitStatusManager::statusFor(const QString &repoPath, const QString &filePath) const {
    ...
    if (it == m_status.end()) return GitFileStatus::Unchanged;
    QString rel = filePath;
    if (rel.startsWith(repoPath))
        rel = rel.mid(repoPath.size()).remove(QRegularExpression("^/+"));
    auto fileIt = it->find(rel);
    if (fileIt == it->end()) return GitFileStatus::Unchanged;
    return fileIt.value();
}

QStringList GitStatusManager::trackedFiles(const QString &repoPath) const {
    auto it = m_status.find(repoPath);
    if (it == m_status.end()) return {};
    return it->keys();
}
