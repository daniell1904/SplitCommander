...
FooterWidget::FooterWidget(QWidget *parent) : QWidget(parent)
{
    ...
    countLbl->setStyleSheet(QString("color:%1;font-size:10px;background:transparent;")
                            .arg(TM().colors().textPrimary));
    selectedLbl = new QLabel();
    selectedLbl->setStyleSheet(QString("color:%1;font-size:10px;background:transparent;")
                                .arg(TM().colors().textAccent));
    selectedLbl->hide();
    sizeLbl = new QLabel();
    sizeLbl->setStyleSheet(QString("color:%1;font-size:10px;background:transparent;")
                           .arg(TM().colors().textPrimary));
    ...
    pvSide->setStyleSheet(QString("background:%1;border:none;").arg(TM().colors().bgBox));
    ...
    div->setFixedWidth(1);
    div->setStyleSheet(QString("background:%1;").arg(TM().colors().bgDeep));
    divLay->addWidget(div);

    auto *infoSide = new QWidget();
    infoSide->setStyleSheet(QString("background:%1;border:none;").arg(TM().colors().bgBox));
    auto *infoLay = new QVBoxLayout(infoSide);
    infoLay->setContentsMargins(8, 8, 8, 8);
    infoLay->setSpacing(4);
    previewInfo = new QLabel();
    previewInfo->setStyleSheet(QString("color:%1;font-size:10px;background:transparent;border:none;")
                               .arg(TM().colors().textPrimary));
